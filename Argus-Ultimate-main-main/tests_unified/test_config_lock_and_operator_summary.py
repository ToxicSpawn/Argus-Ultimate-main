...
def test_config_lock_and_operator_summary_written() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        operator_path = td_path / "operator_summary.json"
        ...
        lifecycle = AutoProvingLifecycle(
            ...
            operator_summary_path=operator_path,
        )
        entry = ExecutionEntrypoint(runtime, manifest_hash="sha256:test-manifest", proving_lifecycle=lifecycle)
        entry.submit_order(
            symbol="BTC/AUD",
            side="buy",
            quantity=1.0,
        ...
        assert "CONFIG_LOCKED" in journal_text
        payload = json.loads(operator_path.read_text(encoding="utf-8"))
        assert payload["run_id"] == "sha256:test-manifest"
        assert payload["config_hash"].startswith("sha256:")
        assert payload["config_hash"] != "sha256:test-manifest"
